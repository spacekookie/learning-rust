...
* Rust doesn't provide a default runtime

* 

  + tokio :: A popular async ecosystem with HTTP, gRPC, and tracing
    frameworks.
  + async-std/ smol :: A project that provides asynchronous
    counterparts to standard library components.
  + embassy :: Rust Embedded async executor and HALs
  + fuchsia-async :: Google's Fuchsia OS executor

* The smallest runtime: ~async-task~ & ~futuress-lite~

  #+INCLUDE: "exercises/runtimes/src/bin/async-task.rs" src rust
...
* ~async-std~
  ...
  #+INCLUDE: "exercises/runtimes/src/bin/async-std.rs" src rust

* ~tokio~

  We can easily create tasks without the syntactic sugar
  
  #+INCLUDE: "exercises/runtimes/src/bin/tokio.rs" src rust
