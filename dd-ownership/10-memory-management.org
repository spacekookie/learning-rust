...
* Garbage collection
  ...
  + Instead of making the user free memory themselves, "we" (the
    program) count references
  + When no more references to a piece of memory exist, we clean it up
  + This is done by a special runtime called _Garbage collector_

* Sceranio 3: garbage collection
...
* Scenario 3: garbage collection
  ...
  At some point the garbage collector cleans of ~foo~ and the string
...
* Summary
  ...
  + Either very explicit or hidden, depending on programming language
  + What does Rust do?  Neither of the two introduced approaches!
